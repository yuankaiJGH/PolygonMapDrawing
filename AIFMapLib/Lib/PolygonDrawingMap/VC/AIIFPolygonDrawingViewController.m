...
@property (strong, nonatomic) AIFMapNavigationView *navigationView;
@property (strong, nonatomic) UIButton *drawBtn;
@property (strong, nonatomic) AIFBaseMapDrawTopInfoView *topInfoView;
@property (nonatomic, strong) UIButton *revocationBtn;//绘制基地撤回
@end
...
- (void)configUI{
    ...
    [self.drawBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(100*iPhone6BasicWidth);
        make.height.mas_equalTo(40*iPhone6BasicHeight);
    }];
    
    [self.view addSubview:self.revocationBtn];
    [self.revocationBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.view).offset(-20);
        make.top.equalTo(self.topInfoView.mas_bottom).offset(50);
        make.width.mas_equalTo(50);
        make.height.mas_equalTo(35);
    }];
    
    self.drawBtn.layer.cornerRadius=5.f;
    self.drawBtn.layer.masksToBounds = YES;
    
    self.revocationBtn.layer.cornerRadius=5.f;
...
- (void)setupEvents{
    ...
    [self.revocationBtn addTarget:self action:@selector(revocationBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.navigationView setSearchAddressShow:YES];
...
- (void)drawBtnAction:(UIButton *)sender{
    ...
    [self.navigationView setSearchAddressShow:NO];
    self.topInfoView.hidden = NO;
}

- (void)revocationBtnAction:(UIButton *)sender{
    [self.drawStateArray removeLastObject];
    AIFBaseDrawViewStateObject *lastState = self.drawStateArray.lastObject;
    if (lastState) {
        [self.drawPointArray removeAllObjects];
        [self.drawPointArray addObjectsFromArray:lastState.orignalPoints];
        self.closeOutline = lastState.close;
        [self updateEditPointArraySaveState:NO checkOutline:YES];
    }else{
        if (self.drawPointArray.count) {
            if (!self.closeOutline) {
                [self.drawPointArray removeLastObject];
            }
            self.closeOutline = NO;
            [self updateEditPointArraySaveState:NO checkOutline:YES];
        }else{
//            [HETMBProgressHUD showHudAutoHidenWithMessage:@"没有可以撤销的操作了"];
        }
    }
}
...
- (void)updateEditPointArraySaveState:(BOOL)saveState checkOutline:(BOOL)check{
    ...
    self.topInfoView.confirmBtn.enabled = stateObj.close;
...
- (void)accountOutlineInfo{
    ...
    NSString *str = [UIConfig takeUpToTwoDecimalPlaces:areaMu];
//    self.pointDrawAreaMu = [NSNumber numberWithFloat:str.floatValue];
    ...
    NSString *str1 = [UIConfig takeUpToTwoDecimalPlaces:distance];
...
- (void)leftBarItemClick:(UIButton *)sender{
...
- (AIFMapNavigationView *)navigationView{
    if (!_navigationView) {
        _navigationView = [AIFMapNavigationView getMapNaivigationView];
        WEAKSELF
        _navigationView.backBlock = ^{
            [weakSelf leftBarItemClick:nil];
        };
        _navigationView.searchBlock = ^{
            [weakSelf searchBarClick:nil];
        };
    }
...
{
    if (!_revocationBtn) {
        _revocationBtn = [UIButton new];
        [_revocationBtn setTitle:@"撤回" forState:UIControlStateNormal];
        [_revocationBtn setBackgroundImage:[UIConfig imageWithColor:THEMECOLOR] forState:UIControlStateNormal];
    }
    return _revocationBtn;
}

- (AIFBaseMapDrawTopInfoView *)topInfoView
{
    if (!_topInfoView) {
        _topInfoView = [AIFBaseMapDrawTopInfoView getMapDrawTopView];
        _topInfoView.confirmBlock = ^{
            AIFLog(@"完成")
        };
    }
    return _topInfoView;
